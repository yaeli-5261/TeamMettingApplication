...
//       console.log("File URL for summarization:", downloadResponse)
...
export const FileUploader = () => {
  ...
  // תיקון הלולאה האינסופית - פיצול ה-useEffect
  useEffect(() => {
    if (meetingId && (!meeting || meeting.id !== Number(meetingId))) {
      dispatch(fetchMeetingsByTeam({ teamId: meeting?.teamId || 0 }))
    }
  }, [meetingId, dispatch]) // הסר את meeting ו-meetings מהתלויות

  useEffect(() => {
    const foundMeeting = meetings.find((m) => m.id === Number(meetingId))
    if (foundMeeting && foundMeeting.id !== meeting?.id) {
      setMeeting(foundMeeting)
  ...
  const getTranscriptDownloadUrl = async (s3Key: string): Promise<string> => {
    try {
      ...
      return s3Key // fallback to original URL
    }
  }
  ...
  const handleSummarize = async (fileUrl: string) => {
    ...
    try {
      ...
      const fileMetadata = {
        MeetingId: Number(meetingId),
        FileUrl: s3Key, // שמור את ה-key במקום URL מלא
        IsTranscript: true,
      }
      ...
      setAiDownloadUrl(s3Key) // שמור את ה-key
      setAiProcessingStatus("success")
      console.log("Summary file linked to meeting successfully:", fileMetadata)
        ...
        <CardContent sx={{ p: { xs: 2, sm: 4 } }}>
          ...
          {!downloadUrl && !hasExistingFile ? (
            ...
            <Box>
              ...
              {/* File Sharing */}
              {downloadUrl && <FileShare fileUrl={downloadUrl || ""} fileName={fileName} />}
            </Box>
          )}
        </CardContent>
      </Card>
